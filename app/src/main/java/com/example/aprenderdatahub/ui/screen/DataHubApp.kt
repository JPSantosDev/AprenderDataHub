...
fun DataHubApp(viewModel: DataHubViewModel = viewModel()) {
    ...
    LaunchedEffect(state.fallBack) {
        if (state.fallBack) {
            snackbar.showSnackbar("Dados carregados do JSON local")
        }
    }
    ...
    Scaffold(
        ...
        if (selected == null) {
            HubListScreen(
                ...
                onForceFallback = { viewModel.refresh(forceError = true) },
                onEmpty = viewModel::simulateEmpty,
